...
<body class="antialiased bg-white text-slate-900 font-sans selection:bg-slate-900 selection:text-white flex flex-col min-h-screen" 
    ...
    <!-- ============ NAVBAR ============ -->
    <nav class="sticky top-0 z-50 bg-white/85 backdrop-blur-xl border-b border-slate-200 transition-all duration-300">
        <div class="max-w-7xl mx-auto px-6 h-20 relative flex items-center justify-between">
            ...
            <!-- Center Links with Dynamic ScrollSpy Active Indicator (Simetris Sempurna) -->
            <div class="hidden lg:flex items-center gap-8 text-xs font-black uppercase tracking-wider absolute inset-y-0 left-1/2 -translate-x-1/2">
                <a href="{{ route('home') }}#beranda" 
                   @click="activeSection = 'beranda'"
                   :class="activeSection === 'beranda' ? 'text-slate-900 border-slate-900' : 'text-slate-400 hover:text-slate-700 border-transparent'"
                   class="inline-flex items-center border-b-2 pb-1 transition-all duration-200">
                    {{ __('messages.nav_home') }}
                </a>
                <a href="{{ route('home') }}#keunggulan" 
                   @click="activeSection = 'keunggulan'"
                   :class="activeSection === 'keunggulan' ? 'text-slate-900 border-slate-900' : 'text-slate-400 hover:text-slate-700 border-transparent'"
                   class="inline-flex items-center border-b-2 pb-1 transition-all duration-200">
                    {{ __('messages.nav_features') }}
                </a>
                <a href="{{ route('home') }}#tentang-kami" 
                   @click="activeSection = 'tentang-kami'"
                   :class="activeSection === 'tentang-kami' ? 'text-slate-900 border-slate-900' : 'text-slate-400 hover:text-slate-700 border-transparent'"
                   class="inline-flex items-center border-b-2 pb-1 transition-all duration-200">
            ...
            <div class="hidden lg:flex items-center gap-3 justify-end z-10">
                ...
                @if (Route::has('login'))
                    @auth
                        <div class="relative" x-data="{ userMenu: false }">
                            <button @click="userMenu = !userMenu" @click.away="userMenu = false" class="flex items-center gap-2 px-3.5 py-1.5 rounded-full border border-slate-300 hover:border-slate-400 hover:bg-slate-50 transition-all focus:outline-none shadow-2xs">
                    ...
                    @else
                        <div class="flex items-center">
                            <a href="{{ route('login') }}" class="px-6 py-2 bg-black hover:bg-slate-800 text-white text-xs font-black uppercase tracking-wider rounded-xl transition-all duration-200 shadow-xs hover:shadow-md active:scale-95 flex-shrink-0 inline-flex items-center justify-center">
        ...
        <div x-show="mobileMenu" x-cloak
             ...
             class="absolute top-full left-0 right-0 z-50 lg:hidden border-t border-slate-200 bg-white/95 backdrop-blur-xl px-6 py-5 space-y-4 text-xs font-bold uppercase tracking-wider shadow-2xl rounded-b-3xl max-h-[calc(100vh-6rem)] overflow-y-auto">
            ...
            <div class="pt-4 border-t border-slate-200 space-y-2">
                @auth
                    <div class="p-3.5 bg-white rounded-2xl border border-slate-200 mb-3 shadow-2xs">
                        <span class="text-xs font-black uppercase tracking-widest text-slate-500 block">AKUN ANDA</span>
                        <p class="text-xs font-bold text-slate-900 truncate">{{ auth()->user()->name }}</p>
                        <span class="inline-block text-xs font-bold uppercase tracking-wider px-2 py-0.5 mt-1 rounded bg-slate-900 text-white">
