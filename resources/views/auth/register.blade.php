...
<body class="h-full bg-cover-cinematic flex items-center justify-center p-4 relative">
    ...
    <div class="relative z-10 w-full max-w-[390px] glass-card rounded-[28px] p-6 sm:p-7 flex flex-col my-auto">
        ...
        {{-- Tab Navigation --}}
        <div class="flex items-center justify-center gap-10 border-b border-white/10 mb-4 pb-0.5">
            <a href="{{ route('login') }}" class="text-white/45 hover:text-white/80 font-semibold text-sm pb-2.5 border-b-2 border-transparent transition-all">
                Sign In
            </a>
        ...
        {{-- Error Alerts --}}
        @if ($errors->any())
            <div class="mb-3 px-3.5 py-1.5 rounded-xl bg-rose-500/20 border border-rose-500/30 text-rose-200 text-xs font-medium">
        ...
        <form method="POST" action="{{ route('register') }}" class="space-y-2.5" data-turbo="false">
            ...
            {{-- Full Name --}}
            <div>
                <label for="name" class="block text-[11px] font-medium text-white/70 mb-1">Nama Lengkap</label>
                <div class="relative">
                    ...
                    <input id="name" type="text" name="name" value="{{ old('name') }}"
                           ...
                           class="input-glass pl-10 pr-3.5 py-2">
                </div>
            </div>

            {{-- Phone / WA --}}
            <div>
                <label for="phone" class="block text-[11px] font-medium text-white/70 mb-1">WhatsApp</label>
                <div class="relative">
                    <div class="absolute left-3.5 top-1/2 -translate-y-1/2 text-white/40 pointer-events-none">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input id="password" type="password" name="password"
                           ...
                           class="input-glass pl-10 pr-10 py-2">
                    <button type="button" onclick="togglePass('password', this)" class="absolute right-3.5 top-1/2 -translate-y-1/2 text-white/40 hover:text-white transition-colors">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {{-- Confirm Password --}}
            <div>
                <label for="password_confirmation" class="block text-[11px] font-medium text-white/70 mb-1">Konfirmasi Password</label>
                <div class="relative">
                    <div class="absolute left-3.5 top-1/2 -translate-y-1/2 text-white/40 pointer-events-none">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <input id="password_confirmation" type="password" name="password_confirmation"
                           required placeholder="••••••••"
                           class="input-glass pl-10 pr-10 py-2">
                    <button type="button" onclick="togglePass('password_confirmation', this)" class="absolute right-3.5 top-1/2 -translate-y-1/2 text-white/40 hover:text-white transition-colors">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                        </svg>
        ...
        {{-- Footer link --}}
        <p class="text-center text-[11.5px] text-white/55 mt-3 font-normal">
            Already have an account? 
            <a href="{{ route('login') }}" class="text-white font-bold hover:underline ml-0.5">Sign in</a>
        </p>
